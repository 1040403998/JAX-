...
class RNNCore(object):
    ...
    def __call__(self, inputs, prev_state, params) -> Tuple[Any, Any]:
        ...
        输入参数:
            inputs : 用于训练模型的 2 维数组 
                形状为 (批大小 B, 单一时间步内输入数据的维数 N) 
            prev_state : 前一时刻循环网络的隐状态
            params : 模型中所需的参数
        
        函数返回: 
            一个形如 (outputs, next_state) 的元组
            outputs: 循环神经网络隐藏层的输出，形状为 (批大小, 隐藏层的维数)
...
class VanillaRNN(RNNCore):
    ...
    def __call__(self, inputs, prev_state, params) -> Tuple[Any, Any]:
        ...
        outputs = jax.nn.tanh(Wu + Mv + b)  # bi, bi, i -> bi
        next_state = outputs
...
class LSTM(RNNCore):
    ...
    def __call__(self, inputs, prev_state, params) -> Tuple[Any, Any]:
        ...
        math::
            \begin{array}{ll}
            i_t = \sigma(W_{ii} x_t + M_{hi} h_{t-1} + b_i) \\
            f_t = \sigma(W_{if} x_t + M_{hf} h_{t-1} + b_f) \\
            g_t = \tanh(W_{ig} x_t + M_{hg} h_{t-1} + b_g) \\
            o_t = \sigma(W_{io} x_t + M_{ho} h_{t-1} + b_o) \\
        ...
        模型输入：
            input      : shape = (B, N)
            prev_state : NamedTuple(hidden, cell)
                hidden.shape = (B, hidden_size)
                cell.shape   = (B, hidden_size)

            params     : 包含3个 jax 的数组 (W, M, b)
                W = (W_ii, W_if, W_io, W_ig)
                M = (M_hi, M_hf, M_ho, M_hg)
                b = (b_hi, b_hf, b_ho, b_hg)
                W.shape = (hidden_size * 4, N)
                M.shape = (hidden_size * 4, hidden_size) 
                b.shape = (hidden_size * 4, )
                这里的N是单一时间步内输入数据的维数

        模型输出：
            output     : shape = (B, hidden_size)
            next_state : NamedTuple(hidden, cell)
                hidden.shape = (B, hidden_size)
                cell.shape   = (B, hidden_size)
        """
        W, M, b = params
        Wx = jnp.einsum("in, bn -> bi", W, inputs)
        Mh = jnp.einsum("ij, bj -> bi", M, prev_state.hidden)
        gates = jax.nn.tanh(Wx + Mh + b)  # gates.shape = (B, hidden_size * 4)
        i, f, o, g = jnp.split(gates, indices_or_sections=4, axis = -1)
        i = jax.nn.sigmoid(i)
        f = jax.nn.sigmoid(f + 1)
        o = jax.nn.sigmoid(o)
        g = jax.nn.tanh(g)
        c = f * prev_state.cell + i * g
        h = o * jax.nn.tanh(c)

        outputs, next_state = h, LSTMState(hidden = h, cell = c)
        return outputs, next_state
    ...
    def initial_state(self, batch_size: int):
        ...
        state = LSTMState(hidden = hidden, cell = cell)
        return state
...
class LSTMModel(object):
    ...
    """
    def __init__(self, hidden_size:int, output_size: int):
        self.hidden_size = hidden_size  # 隐藏层的维数
        self.output_size = output_size  # 神经网络输出的维数

    def init(self, rng, inputs):
        """ 返回模型中的所有初始参数 """
        (T, B, N) = inputs.shape
        W_key, M_key, b_key, Wo_key, bo_key = random.split(rng, num=5)

        # 隐藏层参数
        W = random.normal(key=W_key, shape = (self.hidden_size * 4, N))
        M = random.normal(key=M_key, shape = (self.hidden_size * 4, self.hidden_size))
        b = random.normal(key=b_key, shape = (self.hidden_size * 4, ))
        hidden_layer_params = (W, M, b)
    ...
    def apply(self, params, rng, inputs, initial_state=None):
        ...
        # 隐藏层前向传播
        core = LSTM(output_channels=self.hidden_size)
...
model = LSTMModel(hidden_size=10, output_size=1)
