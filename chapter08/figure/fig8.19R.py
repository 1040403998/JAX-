    ...
    for time_steps in range(100):
        ...
        for i in range(100):
            train_loss, params, opt_state = update(None, params, opt_state, 
                        x = train_data[:time_steps+1], y = train_labels[:time_steps+1])
        end_time = time.time()
        time_list.append((end_time - start_time)/100)

        print(time_steps+1, time_list[-1])
        with open("execution_time_list2.txt", mode = "a") as f:
            f.writelines("{}, {} \n".format(time_steps+1, time_list[-1]))
            f.close()
    
    return time_list

# execution_time_list = test_compilation_time(train_data = train_data, train_labels = train_labels, 
#                             test_data = test_data , test_labels  = test_labels)

# 可视化输出
execution_time_list1 = []
time_step_list1 = []
with open("execution_time_list1.txt", "r") as f:
    for line in f.readlines():
        print(line)
        time_steps, execution_time = line.split(",")
        time_step_list1.append(float(time_steps))
        execution_time_list1.append(float(execution_time)*1000)

import matplotlib.pyplot as plt

plt.plot(time_step_list1, execution_time_list1, label = "static unroll", )

# 可视化输出
execution_time_list2 = []
time_step_list2 = []
with open("execution_time_list2.txt", "r") as f:
    for line in f.readlines():
        print(line)
        time_steps, execution_time = line.split(",")
        time_step_list2.append(float(time_steps))
        execution_time_list2.append(float(execution_time)*1000)

import matplotlib.pyplot as plt

plt.plot(time_step_list2, execution_time_list2, label = "dynamic unroll", linestyle = "-.")

plt.grid("-")
plt.xlabel("time steps")
plt.ylabel("training time (ms)")
plt.legend(loc = "upper left")
plt.savefig('execution_time.png', format='png', dpi=200)
plt.close()