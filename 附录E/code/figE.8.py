...
for i in range(int(time_stop / time_step)):
    ...
    n_trace.append(neuron_state[1])
    m_trace.append(neuron_state[2])
    h_trace.append(neuron_state[3])

# 可视化输出
plt.figure()
plt.plot(t_trace, n_trace, c = "blue" , linestyle="-" , label = r"$n(t)$")
plt.plot(t_trace, m_trace, c = "red"  , linestyle="-.", label = r"$m(t)$")
plt.plot(t_trace, h_trace, c = "green", linestyle="--", label = r"$h(t)$")
